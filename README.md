...
本ツールは、**「サーバーが起動しなくなる致命的なミス」** を防ぐことに特化しています。そのため、以下のルールに基づいて設計されています。

### 1. なぜ「スマートクォート」「NBSP」「BOM」だけなのか？
これらは、Word等の文書作成ツールやWebページから設定値をコピー＆ペーストした際に混入する**「見えないゴミ」**であり、人間が目視で見つけることが不可能に近いからです。これらが残っていると、Libertyサーバーは構文エラーを起こし、起動に失敗します。

### 2. なぜ「コメント」や「空行」「CDATA」は無視するのか？
これらは XML の仕様として **「有効（Valid）」** な記述だからです。
* **CDATA:** 特殊文字を含むSQLやスクリプトが記述されている可能性があり、安易に検査・変更するとアプリケーションの動作に影響を与えるリスクがあります。
* **空行・コメント:** これらがあってもサーバーの動作には影響しません。本ツールは「安全第一」とし、設定内容そのもの（値や構造）には一切触れません。

結果として、**`05_messy_format.xml` のような「見た目は汚くても、XMLとして壊れていないファイル」は、本ツールでは「正常（OK）」と判定されます。**